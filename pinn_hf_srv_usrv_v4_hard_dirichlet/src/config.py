...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if str(config["runtime"].get("device", "cpu")).lower() != "cpu":
        raise ValueError("v3 partitioned-MLP 项目强制 CPU-only，请将 runtime.device 设置为 cpu。")
    if str(config["runtime"].get("dtype", "float64")).lower() != "float64":
        raise ValueError("v3 默认并强制使用 float64，以减轻极端扩散系数下的数值噪声。")
    model_cfg = config["model"]
    if int(model_cfg.get("input_dim", -1)) != 3:
        raise ValueError("网络输入维度必须为 3，仅允许 x_hat/y_hat/t_hat。")
    if str(model_cfg.get("constraint_mode", "")).lower() not in {"ic_hard", "ic_dirichlet_hard"}:
        raise ValueError("v4 only supports constraint_mode='ic_hard' or 'ic_dirichlet_hard'.")
    physics_mode = str(config["physics"].get("mode", "")).lower()
    if physics_mode != "effective_diffusion":
        raise ValueError("v3 只允许 physics.mode='effective_diffusion'。")
