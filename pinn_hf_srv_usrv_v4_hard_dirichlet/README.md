...
This project is copied from `pinn_hf_srv_usrv_v3_partition_mlp` and keeps the same HF/SRV/USRV partitioned-MLP structure, geometry, sampling logic, and training workflow. The main difference is that the production-end Dirichlet pressure boundary is enforced as a hard output constraint instead of a weighted soft loss.
