...
def line_hf_srv_residual(
    ...
    """Pressure coupling between lower-dimensional HF lines and nearby SRV.

    In v12, HF has no area. A conventional two-sided rectangle interface is
    therefore not available. We instead sample points exactly on the fracture
    centerline, offset them by eps along the sampled normal into SRV, and
    penalize the pressure jump between the explicit HF subnet and the SRV subnet.
    """

    if eps <= 0.0:
        raise ValueError(f"line HF-SRV offset eps must be positive, got {eps:g}.")
    if xyt_line.numel() == 0:
        empty2 = xyt_line.new_zeros((0, 2))
