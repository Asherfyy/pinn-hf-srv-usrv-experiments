"""IDE 直接运行：绘制 v3 partitioned-MLP 压力云图。
...
def relaunch_with_venv_if_needed() -> None:
    ...
    v3 项目可以独立创建自己的 `.venv`。如果 `.venv` 不存在，就使用 IDE 当前解释器，
    这样不会强行依赖其他项目环境。
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="绘制 v3 partitioned-MLP HF/SRV/USRV PINN 压力云图。")
