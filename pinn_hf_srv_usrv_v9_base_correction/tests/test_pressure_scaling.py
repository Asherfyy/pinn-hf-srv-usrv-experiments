...
def test_pressure_round_trip() -> None:
    ...
    assert torch.max(torch.abs(restored[:, 0:1] - p12)).item() < 1.0e-5
    assert torch.max(torch.abs(restored[:, 1:2] - p13)).item() < 1.0e-5
...
def test_dirichlet_target_scale() -> None:
    ...
    assert torch.max(torch.abs(dirichlet_target_hat(t0, config["boundary"]) - 1.0)).item() < 1.0e-6
    t_large = torch.full((4, 1), 1000.0, dtype=dtype)
    target = dirichlet_target_hat(t_large, config["boundary"])
    assert torch.max(torch.abs(target[:, 0:1] - target[:, 1:2])).item() < 1.0e-6
