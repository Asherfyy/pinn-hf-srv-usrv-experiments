...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if str(config["runtime"].get("dtype", "float32")).lower() != "float32":
        raise ValueError("v9 默认并强制使用 float32。")
    ...
    if str(model_cfg.get("constraint_mode", "")).lower() not in {"ic_hard", "ic_base_correction"}:
        raise ValueError("v9 only supports constraint_mode='ic_hard' or 'ic_base_correction'.")
