...
v2 项目刻意把几何、物理系数、采样数量和训练超参数集中到 YAML 中。这样做的原因
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if str(config["runtime"].get("device", "cpu")).lower() != "cpu":
        raise ValueError("v2 简化项目强制 CPU-only，请将 runtime.device 设置为 cpu。")
    if str(config["runtime"].get("dtype", "float64")).lower() != "float64":
        raise ValueError("v2 默认并强制使用 float64，以减轻极端扩散系数下的数值噪声。")
    ...
    if str(model_cfg.get("constraint_mode", "")).lower() != "ic_hard":
        raise ValueError("v2 只实现 ic_hard，不保留 direct/dirichlet_hard 分支。")
    physics_mode = str(config["physics"].get("mode", "")).lower()
    if physics_mode != "effective_diffusion":
        raise ValueError("v2 只允许 physics.mode='effective_diffusion'。")
