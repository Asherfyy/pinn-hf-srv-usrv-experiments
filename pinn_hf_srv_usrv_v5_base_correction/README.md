...
This project is copied from `pinn_hf_srv_usrv_v3_partition_mlp` and keeps the same HF/SRV/USRV geometry, sampling, losses, plotting, and training workflow. The main change is the output structure:

```text
u = u_base(x, y, t) + envelope(t) * correction_NN(x, y, t)
```

By default `u_base` is the initial uniform pressure field. If `model.base_checkpoint` is set in `config/default.yaml`, `u_base` is evaluated by that frozen checkpoint at `max(t - base_time_lag_days, t_min)`, so the trainable network learns a correction relative to a previous-stage or previous-time field.
