...
def build_edfm_grid(geometry: ReservoirGeometry, config: dict[str, Any]) -> EdfmGrid:
    ...
    cell_xy, cell_region, cell_volume, cell_phi, cell_perm, cell_conductivity = _build_matrix_cells(geometry, config, x_edges, y_edges, thickness, component_count)
    ...
    if fracture_segments:
        ...
        frac_perm = np.full((len(fracture_segments),), _region_representative_conductivity(config, REGION_HF), dtype=np.float64)
    ...
    cell_storage = _cell_storage(config, cell_phi, cell_volume)
...
def _build_matrix_cells(
    ...
    perm = np.where(in_srv, _region_representative_conductivity(config, REGION_SRV), _region_representative_conductivity(config, REGION_USRV)).astype(np.float64)
    ...
    return xy, region, volume, phi, perm, conductivity
...
def _parameterization(config: dict[str, Any]) -> str:
    return str(config["physics"].get("parameterization", "darcy")).lower()
...
def _region_storage_phi(config: dict[str, Any], region: str) -> float:
    if _parameterization(config) == "legacy_diffusion":
        return float(config["physics"]["Fai"][region])
    if region == REGION_HF:
        return float(config["rock"]["porosity"]["HF"])
    return float(config["rock"]["porosity"]["matrix"])
...
def _region_component_conductivity(config: dict[str, Any], region: str, component_count: int) -> np.ndarray:
    ...
    rock = config["rock"]
    if region == REGION_HF:
        base = float(rock["permeability_mD"]["HF"])
    else:
        base = float(rock["permeability_mD"][region])
    multipliers = np.asarray(config["pressure"].get("transmissibility_multipliers", [1.0] * component_count), dtype=np.float64)
...
def _cell_storage(config: dict[str, Any], cell_phi: np.ndarray, cell_volume: np.ndarray) -> np.ndarray:
    if _parameterization(config) == "legacy_diffusion":
        return cell_phi * cell_volume
    return cell_phi * float(config["rock"]["total_compressibility_per_MPa"]) * cell_volume
...
def _transmissibility_scale(config: dict[str, Any]) -> float:
    ...
    return base / float(config["fluid"]["viscosity_cP"])
