"""Helpers for attaching a frozen base field model to the v13 correction PINN."""
...
def attach_base_model_from_config(model: PINNModel, config: dict[str, Any], device: torch.device, dtype: torch.dtype, config_path: str | Path) -> None:
    """Attach an optional frozen checkpoint as u_base.

    When model.base_checkpoint is null, the model falls back to the uniform
    initial pressure field as its baseline.
    """
