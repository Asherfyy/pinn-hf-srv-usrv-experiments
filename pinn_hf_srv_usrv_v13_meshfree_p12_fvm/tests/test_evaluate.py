...
def make_context() -> tuple[dict, ReservoirGeometry, PINNModel, torch.device, torch.dtype]:
    ...
    config["fvm_reference"]["enabled"] = False
    config["evaluation"]["adaptive_plot"].update(
        {"coarse_nx": 31, "coarse_ny": 21, "srv_nx": 41, "srv_ny": 31, "hf_long_axis_points": 80, "hf_short_axis_points": 1, "transition_band_points": 30}
    )
...
def test_triangulation_region_mask_and_pressure_finite() -> None:
    ...
    assert set(field) >= {"x", "y", "P12", "region"}
...
def test_diagnostics_table_can_be_generated() -> None:
    ...
    metrics = set(diagnostics["metric"].tolist())
    assert "loss_fvm_teacher" not in metrics
    assert "srv_usrv_jump_left_rms_mpa_t1000" in metrics
    assert "srv_usrv_jump_bottom_rms_mpa_t1000" in metrics
    assert "srv_usrv_jump_top_rms_mpa_t1000" in metrics
