...
本项目由 `pinn_hf_srv_usrv_v3_partition_mlp` 复制而来，几何、采样、分区 MLP 和训练入口基本保持一致；核心变化是把 PDE 从压力强形式改为混合压力-通量形式。网络输出从 `u12,u13` 扩展为 `u12,u13,q12_x,q12_y,q13_x,q13_y`，PDE loss 包含通量定义残差 `q + kappa*grad(u)=0` 和守恒残差 `u_t + div(q)=0`，因此不再直接对压力 `u` 求二阶导。
