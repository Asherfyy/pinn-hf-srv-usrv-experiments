...
class ConstantICModel(PINNModel):
    ...
    def forward_normalized(self, z: torch.Tensor) -> torch.Tensor:
        one = z[:, 0:1] * 0.0 + 1.0
        zero = z[:, 0:1] * 0.0
        return torch.cat([one, one, zero, zero, zero, zero], dim=1)
...
def test_pde_residual_shapes_and_constant_solution() -> None:
    ...
    darcy12, darcy13, conservation12, conservation13 = pde_residual(model, xyt, "SRV", config["physics"], config)
    assert darcy12.shape == (2, 2)
    assert darcy13.shape == (2, 2)
    assert conservation12.shape == (2, 1)
    assert conservation13.shape == (2, 1)
    assert torch.max(torch.abs(darcy12)).item() < 1.0e-12
    assert torch.max(torch.abs(darcy13)).item() < 1.0e-12
    assert torch.max(torch.abs(conservation12)).item() < 1.0e-12
    assert torch.max(torch.abs(conservation13)).item() < 1.0e-12
