...
PROJECT_VERSION = "partition_mlp_v6_mixed_flux"
...
def get_torch_dtype(dtype_name: str) -> torch.dtype:
    """把配置字符串转换为 torch dtype；v6 仅接受 float64。"""

    if str(dtype_name).lower() != "float64":
        raise ValueError(f"v6 只支持 float64，当前 dtype={dtype_name}。")
...
def pressure_hat_to_mpa(u: torch.Tensor, boundary_cfg: dict[str, Any]) -> torch.Tensor:
    ...
    if u.ndim != 2 or u.shape[1] < 2:
        raise ValueError(f"pressure_hat_to_mpa 期望至少 [N,2]，当前 {tuple(u.shape)}。")
    pressure = u[:, 0:2]
    params = pressure_affine_parameters(boundary_cfg)
    p12 = pressure[:, 0:1] * params["p12_scale_mpa"] + params["p12_offset_mpa"]
    p13 = pressure[:, 1:2] * params["p13_scale_mpa"] + params["p13_offset_mpa"]
