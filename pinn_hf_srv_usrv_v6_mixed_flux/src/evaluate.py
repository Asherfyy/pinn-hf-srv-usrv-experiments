...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成 v6 mixed-flux partitioned-MLP 诊断表。")
...
def compute_diagnostics(config: dict[str, Any], geometry: ReservoirGeometry, model: PINNModel, device: torch.device, dtype: torch.dtype) -> pd.DataFrame:
    ...
    rows: list[dict[str, float | str | int]] = [
        ...
        {"metric": "neumann_q12n_rms", "value": float(torch.sqrt(torch.mean(dn12.detach() ** 2)).cpu())},
        {"metric": "neumann_q13n_rms", "value": float(torch.sqrt(torch.mean(dn13.detach() ** 2)).cpu())},
    ]
