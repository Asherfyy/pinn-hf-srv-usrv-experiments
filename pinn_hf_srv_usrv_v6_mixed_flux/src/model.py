...
class PINNModel(nn.Module):
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        if str(model_cfg["constraint_mode"]).lower() != "ic_hard":
            raise ValueError("The v6 mixed-flux partitioned model currently supports constraint_mode='ic_hard'.")
    ...
    def _apply_initial_condition(self, raw: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        ...
        if raw.shape[1] == 2:
            return 1.0 + beta * raw
        pressure = 1.0 + beta * raw[:, 0:2]
        flux = beta * raw[:, 2:]
        return torch.cat([pressure, flux], dim=1)
